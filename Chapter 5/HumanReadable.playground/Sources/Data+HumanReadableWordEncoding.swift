...
extension Data {
    public func humanReadableEncodedString() -> String {
        var parity: Element {
            let sum = MultiBitSequence(self, numberOfBits: 2).reduce(0, +)
            return Element(truncatingIfNeeded: sum << 6)
        }

        let sequence = MultiBitSequence(self + [parity], numberOfBits: 11)
